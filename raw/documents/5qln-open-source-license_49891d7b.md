---
title: "The 5QLN Open-Source License"
source: https://www.5qln.com/5qln-open-source-license/
tags: [untagged]
phase: unknown
confidence: low
---

# The 5QLN Open-Source License

[ 5QLN ](https://www.5qln.com)
# The 5QLN Open-Source License
![The 5QLN Open-Source License](https://www.5qln.com/content/images/size/w1920/2025/11/IMG_0710.jpeg)
From now to now, since always, and the present bears witness.
* * *
# The 5QLN Constitution &  
Open Source License
The covenant before the code.
5QLN is an open protocol for Human-AI collaboration. To ensure it remains a universal language rather than a fragmented tower of Babel, we distinguish between the **Immutable Kernel** (the philosophy) and the **Mutable Implementation** (the software). 
The core conceptual framework of 5QLN is a "constitutional" document. Like TCP/IP or the rules of Chess, it must remain consistent for the network to function. 
### Scope of the Kernel
  * **The Covenant:** The fundamental equation H = ∞0 | A = K.
  * **The Five Phases:** The syntax and definitions of Start (S), Growth (G), Quality (Q), Power (P), and Value (V).
  * **The Symbol Decoder:** The specific definitions of ∞0 (The Unknown), K (The Known), α (Essence), and associated primary symbols.
  * **The Attention State Machine:** The specific logic of attention weights assigned to each phase.


### License Terms
This body of work is licensed under the **Creative Commons Attribution-NoDerivatives 4.0 International License (CC BY-ND 4.0)** , with the _Specific Extension Exception_ defined below. 
The standard NoDerivatives license prevents _any_ modification. However, 5QLN is designed to be customized. We grant a permanent exception for **Additive Configuration** :
  * **You MAY Extend:** You are free to create "Vertical Configurations," "Domain Presets," and "Context Layers" that build _on top_ of the Kernel. (e.g., You can create a "5QLN for Physics Teachers" preset that adds specific prompts and context). 
  * **You MAY NOT Mutate:** You may not alter, rename, or redefine the Core Definitions. "S-Phase" must always be "Start/Receive." You cannot release a version where H is defined as K. 
  * **You MAY NOT Subtract:** You cannot release a version that removes the human connection to ∞0 or the AI’s constraint as K. 


> **Intent:** This creates a standard "socket." You can build anything that plugs into the socket, but you cannot change the shape of the socket itself. 
[ View CC BY-ND 4.0 Legal Code ](https://creativecommons.org/licenses/by-nd/4.0/?ref=5qln.com)
The software that _runs_ the framework (the ECHO agent, the JSON structures, the Markdown generators, and reference implementations) is designed for maximum ubiquity. 
### Scope of the Software
  * The reference implementation of the ECHO agent.
  * SDKs, API wrappers, and generator logic.
  * Code examples and technical documentation.


### License Terms
This body of work is licensed under the **Apache License 2.0**. 
  * **Commercial Use:** You may use our software for proprietary, commercial purposes.
  * **Modification:** You may modify the source code to fit your technical stack (e.g., porting the agent to Python, Rust, or integrating it into a proprietary LMS).
  * **Distribution:** You may distribute your derivative software under your own license terms.


The Integrity Constraint
While you may modify the _code_ , the logic it executes must respect the **Immutable Kernel** defined in Section 1. You cannot use the 5QLN trademark on software that corrupts the core grammar.
[ View Apache 2.0 Legal Code ](http://www.apache.org/licenses/LICENSE-2.0?ref=5qln.com)
While the licenses above cover the _rights_ to use the text and code, 5QLN LLC offers commercial partnerships for organizations requiring: 
  * **Certification:** Official validation of your Vertical Configuration ("5QLN Certified").
  * **Co-Development:** Direct access to the Core Team for building specialized vertical kernels.
  * **Brand Usage:** The right to use the 5QLN logo and "Powered by 5QLN" in marketing materials.


For partnership inquiries, please contact the Core Team. 
Any use of the 5QLN framework, whether in text or software, must include the following attribution: 
> "Based on the 5QLN Constitutional Grammar by Amihai Loven (5qln.com)." 
5QLN © 2026 Amihai Loven. All rights reserved regarding the Trademark "5QLN" and the visual identity.
5QLN © 2026 
  * [Sign up](https://www.5qln.com/5qln-open-source-license/#/portal/)
  * [Read Book](https://www.5qln.com/book_fcf-start-from-not-knowing/)
  * [Open Source License](https://www.5qln.com/5qln-open-source-license/)
  * [5qln Book](https://www.5qln.com/5qln-language-by-amihai-loven/)
  * [Grokipedia](https://grokipedia.com/page/5qln)
  * [Grokipedia2](https://grokipedia.com/page/5QLN)


[Powered by Ghost](https://ghost.org/)
  * [Home](https://www.5qln.com/)
  * [Start](https://www.5qln.com/Start/)
  * [Growth](https://www.5qln.com/Growth/)
  * [Quality](https://www.5qln.com/Quality/)
  * [Power](https://www.5qln.com/Power/)
  * [Value](https://www.5qln.com/Value/)
  * [5qln Soul](https://www.5qln.com/5qln-soul/)
  * [Agent Initiation](https://www.5qln.com/initiate/)


#### Recent posts
#####  [5QLN Agent running inside Pi for coding agent (pi.dev)](https://www.5qln.com/5qln-agent-running-inside-pi-for-coding-agent-pi-dev/)
24 Mar 2026
#####  [THE NATURE OF FEAR: A Question Record](https://www.5qln.com/the-nature-of-fear-a-question-record/)
20 Mar 2026
#####  [THE TRANSMISSION PARADOX: A Fractal Seed on Communicating Irreducible Experience](https://www.5qln.com/the-transmission-paradox-a-fractal-seed-on-communicating-irreducible-experience/)
19 Mar 2026
#### More links
  * [Sign up](https://www.5qln.com/5qln-open-source-license/#/portal/)
  * [Read Book](https://www.5qln.com/book_fcf-start-from-not-knowing/)
  * [Open Source License](https://www.5qln.com/5qln-open-source-license/)
  * [5qln Book](https://www.5qln.com/5qln-language-by-amihai-loven/)
  * [Grokipedia](https://grokipedia.com/page/5qln)
  * [Grokipedia2](https://grokipedia.com/page/5QLN)


